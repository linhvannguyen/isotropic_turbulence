...
kr = GridSearchCV(KernelRidge(kernel='rbf'), cv=5,
                  param_grid={"alpha": np.logspace(-5, -1, 10), "gamma": np.logspace(-10, 0, 10)})
kr.fit(Xl_tr, Xh_tr)
...
gammas_range=np.logspace(-8, -3, 40)
train_MSE_gamma, test_MSE_gamma = validation_curve(KernelRidge(kernel='rbf', alpha=kr.best_estimator_.alpha), 
                                                         Xl_tr, Xh_tr, param_name="gamma", param_range=gammas_range, 
                                                         scoring = "mean_squared_error", cv=10)
print('\n Finish validation curve for gamma:')
                                                         
# Validation curve for lambda
from sklearn.learning_curve import validation_curve
lambdas_range = np.logspace(-7, -1, 40)
train_MSE_lambda, test_MSE_lambda = validation_curve(KernelRidge(kernel='rbf', gamma=kr.best_estimator_.gamma), 
...
io.savemat('/data/ISOTROPIC/regression/KRR_validationcurve.mat', 
           dict(gammas_range=gammas_range, train_MSE_gamma=train_MSE_gamma, test_MSE_gamma=test_MSE_gamma,
           lambdas_range=lambdas_range, train_MSE_lambda = train_MSE_lambda, test_MSE_lambda = test_MSE_lambda))    