...
#define FILENAME_RD "/workdir/er2/nguyen/data/isotropic/NLM/interpdiff/Udiff_spacespacing3.nc"
#define FILENAME_WR "/workdir/er2/nguyen/data/isotropic/NLM/interpdiff/NLM_interpdiff_simpatch06_accumpatch06_searchbox8_tau0100.nc"
...
#define SIM_HAFTSIZE 6 // dont go further than 12
#define ACC_HAFTSIZE SIM_HAFTSIZE
...
#define NUM_BLOCKS 23 /* #(1:SCALE_FACTOR_TIME:N_HR) - 1*/
...
void NLmean(double *x_NLM, double *weight_NLM, double *x_ref, double *x_moving, double *x_fusion, int *gridpatches_y, int *gridpatches_z, int *acc_ids)
{
    ...
    for (int est_idy = 0; est_idy < N_HR; est_idy++)
    {
...
int main()
{
    ...
    char *varnames[NUM_VARS] = {"x_HR_NLM_smallscales_all"};
    ...
    /* Allocate memory */
    double *x_HR_fusion_largescales_all = (double*)malloc(1 * NUM_2DSNAPS * N_HR * N_HR * sizeof(double));
    double *x_HR_fusion_smallscales_all = (double*)malloc(1 * NUM_2DSNAPS * N_HR * N_HR * sizeof(double));
    double *x_NLM_smallscales_all = (double*)malloc(1 * NUM_2DSNAPS * N_HR * N_HR * sizeof(double));
    ...
    {
        /* read all snapshots */
        size_t start_ids[4] = {snap3d_id, 0, 0, 0};
        size_t count_ids[4] = {1, NUM_2DSNAPS, N_HR, N_HR };
        ...
        for(int block_id = 0; block_id < NUM_BLOCKS; block_id++)
        {
            ...
            for (int t = 1; t < SCALE_FACTOR_TIME; t++)
            {
                ...
                printf("\n Complete estimating snapshot id %i in 3D block %i bounded by 2 PIV planes at [%i, %i] in %f seconds \n", t_est, block_id, t_prev, t_after, elapped_time);
            }
            ...
            // Write to file
            write_netcdf(FILENAME_WR, "x_HR_NLM_smallscales_all", start_ids, count_ids, x_NLM_smallscales_all);
        }
    }

    /* free memory */
    free(x_NLM_smallscales); free(W);
    free(x_current_largescales); free(x_first_smallscales);
