function [x_rec,weights] = NLmean2(x_rec,weights, x_ref, x_moving, x_fusion, gridy, gridz, simpatch_haftsize, accpatch_haftsize, neighbor_haftsize, tau)
%NLmean: non-local similarity-based reconstruction: x_fusion is
    ...
    patches=extract_patch(x_fusion, squeeze(gridy(:,:,:,acc_ids)), squeeze(gridz(:,:,:,acc_ids)));
    rows_ref = squeeze(gridy(:,:,ceil(dim_neighbor/2),acc_ids));
    cols_ref = squeeze(gridz(:,:,ceil(dim_neighbor/2),acc_ids));
    x_rec(rows_ref+(cols_ref-1)*Ny) = x_rec(rows_ref+(cols_ref-1)*Ny) + sum(repmat(K,[1 1 1 dim_simpatch]).*extract_patch(x_fusion, squeeze(gridy(:,:,:,acc_ids)), squeeze(gridz(:,:,:,acc_ids))),3);
%     [x_rec, weights]=put_patch(x_rec, weights, patches, rows_ref, cols_ref,K);
