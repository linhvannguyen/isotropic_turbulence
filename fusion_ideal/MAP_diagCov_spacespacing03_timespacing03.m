...
time_spacing=03; % subsampling ration in time (from 40Hz to 4Hz)
...
% left=2*space_spacing; right = 2*space_spacing-(Ny-HTLS_idy(end));
% bottom=2*space_spacing; top = 2*space_spacing-(Nz-HTLS_idz(end));
...
%% Time interp
% left=2*time_spacing; right = 2*time_spacing-(Nx-LTHS_idt(end));
% 
...
%     PIV_sampled=[PIV_sampled(Nx-left+1:1:Nx,:,:); PIV_sampled; PIV_sampled(1:1:right,:,:)];
%     
%     PIV_interp=interp1(LTHS_idt_enlarged,PIV_sampled(LTHS_idt_enlarged,:,:),HTHS_idt_enlarged,'spline');    
%     nc2{'Uinterp'}(t,:,:,:)=PIV_interp(left+1:left+Nx,:,:);
% end
% 
% close(nc1); close(nc2);
% 
...
hs=nc1{'velocity_x'}(:,LTHS_idt,HTHS_idy,HTHS_idz)-nc2{'Uinterp'}(:,LTHS_idt,HTHS_idy,HTHS_idz);
close(nc1); close(nc2);

S=squeeze(sum(hs.^2,1));
S=squeeze(sum(S,1));
...
for i=1:space_spacing
    S_ave_onecol=mean(S(i:space_spacing:Ny,:),1);
    for j=1:space_spacing
        temp=mean(S_ave_onecol(j:space_spacing:Nz));
        S_ave(i:space_spacing:Ny,j:space_spacing:Nz)=temp;
    end  
end
...
for i=1:time_spacing
    ht=nc1{'velocity_x'}(:, i:time_spacing:Nx,HTLS_idy,HTLS_idz)-nc2{'Uinterp'}(:, i:time_spacing:Nx,HTLS_idy,HTLS_idz);
    T_ave(i)=mean(ht(:).^2);
end
close(nc1); close(nc2);
...
nc('Ny')=Ny;
nc('Nz')=Nz;
nc{'Zhat_all'}=ncfloat('Nt','Nx','Ny','Nz');
