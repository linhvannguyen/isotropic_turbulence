...
nc2 = netcdf('/data/ISOTROPIC/DICTIONARYLEARNING/downsample4_3_Elad/SRrec/SR_patchesLRfeatures_patchsize08_all.nc','r');
nc3 = netcdf('/data/ISOTROPIC/DICTIONARYLEARNING/downsample4_3/SRrec/SR_patchesLRprefil_bicubic_patchsize08_all.nc','r');
...
%     X_LR_interp=double(imresize(X_LR_enlarged, scale,'bicubic'));
    ...
    %% ERROR
    error_SRrec_features=X_HR_org-nc2{'X_HR_rec'}(ttds(t),:,:);
    error_SRrec=X_HR_org-nc3{'X_HR_rec'}(ttds(t),:,:);
...
h1=loglog(k_2D_errorLR_interp(1:k_max),E_error_LR_interp_ave(1:k_max),'b-*','LineWidth',2); 
hold on;
h2=loglog(k_2D_errorSR(1:k_max),E_error_SRrec_ave(1:k_max),'m-*','LineWidth',2); 
h3=loglog(k_2D_errorSR(1:k_max),E_error_SRrec_features_ave(1:k_max),'r-*','LineWidth',2); 
...
filename=strcat('spectra2d_error');
...
h1=loglog(k_2D_errorLR_interp(1:k_max),E_error_LR_interp_ave(1:k_max)./E_HR_ave(1:k_max),'b-*','LineWidth',2); 
hold on;
h2=loglog(k_2D_errorSR(1:k_max),E_error_SRrec_ave(1:k_max)./E_HR_ave(1:k_max),'m-*','LineWidth',2); 
h3=loglog(k_2D_errorSR(1:k_max),E_error_SRrec_features_ave(1:k_max)./E_HR_ave(1:k_max),'r-*','LineWidth',2); 
...
filename=strcat('spectra2d_error_normalized');
export_fig(filename,'-eps','-q101','-a4','-nocrop');

%%
error_total_LR=sum(E_error_LR_interp_ave)
error_total_SRrec=sum(E_error_SRrec_features_ave)

error_largescale_LR=sum(E_error_LR_interp_ave(1:k_max))
error_largescale_SRrec=sum(E_error_SRrec_features_ave(1:k_max))

error_smallscale_LR=sum(E_error_LR_interp_ave(1+k_max:end))
error_smallscale_SRrec=sum(E_error_SRrec_features_ave(1+k_max:end))
