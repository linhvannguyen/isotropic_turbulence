...
load(PATCHES_FILENAME, 'patches_af_all','patches_hf_all');
...
% JOINT PATCHES
patches_all = [patches_af_all; patches_hf_all];
...
clearvars patches_af_all patches_hf_all;
...
D_AF = D(1:dim_patch,:);
D_HF = D(dim_patch+1:end,:);

ODL_FILENAME=strcat('/data/ISOTROPIC/dictionary_learning/ideal/space_ratio_03/DICTIONARY_coupleAFHF_K',num2str(params.K,'%.4d'),'_lambda',strrep(num2str(params.lambda,'%.2f'),'.',''),'.mat');
save(ODL_FILENAME,'D_AF','D_HF','CoefMatrix');
