...
%% PCA
C = patches_lf_features_all * patches_lf_features_all';
[V, D] = eig(C);
D = diag(D);
D = cumsum(D) / sum(D);
k = find(D >= 1e-3, 1); % ignore 0.01% energy
dim_pca=numel(D)-k;
V_pca = V(:, end-dim_pca+1:end); % ignore 0.23% energy
patches_lf_features_pca = V_pca' * patches_lf_features_all;
...
save(PATCHES_FILENAME,'patches_af_all','patches_lf_all','patches_hf_all','patches_lf_features_pca','V_pca','-v7.3');